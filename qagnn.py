...
print(
    "gpu: %s"
    % subprocess.check_output("echo $CUDA_VISIBLE_DEVICES", shell=True).decode("utf")
)
...
def main():
    ...
    parser.add_argument(
        "--save_dir", default=f"./saved_models/qagnn/", help="model output directory"
    )
    ...
    # data
    parser.add_argument(
        "--num_relation", default=38, type=int, help="number of relations"
    )
    parser.add_argument(
    ...
    # model architecture
    parser.add_argument(
        "-k", "--k", default=5, type=int, help="perform k-layer message passing"
    )
    parser.add_argument(
        "--att_head_num", default=2, type=int, help="number of attention heads"
    )
    parser.add_argument(
        "--gnn_dim", default=100, type=int, help="dimension of the GNN layers"
    )
    parser.add_argument(
        "--fc_dim", default=200, type=int, help="number of FC hidden units"
    )
    parser.add_argument(
        "--fc_layer_num", default=0, type=int, help="number of FC layers"
    )
    parser.add_argument(
        "--freeze_ent_emb",
        default=True,
    ...
    parser.add_argument(
        "--simple", default=False, type=bool_flag, nargs="?", const=True
    )
    ...
    # regularization
    parser.add_argument(
        "--dropouti", type=float, default=0.2, help="dropout for embedding layer"
    )
    parser.add_argument(
        "--dropoutg", type=float, default=0.2, help="dropout for GNN layers"
    )
    parser.add_argument(
        "--dropoutf", type=float, default=0.2, help="dropout for fully-connected layers"
    )
    ...
    parser.add_argument(
        "-dlr",
        "--decoder_lr",
        default=DECODER_DEFAULT_LR[args.dataset],
        type=float,
        help="learning rate",
    )
...
def train(args):
    ...
    for name, param in model.decoder.named_parameters():
        if param.requires_grad:
            ...
            print(
                "\t{:45}\tfixed\t{}\tdevice:{}".format(name, param.size(), param.device)
            )
        ...
        for epoch_id in range(args.n_epochs):
            ...
            for qids, labels, *input_data in dataset.train():
                ...
                if (global_step + 1) % args.log_interval == 0:
                    ...
                    print(
                        "| step {:5} |  lr: {:9.7f} | loss {:7.4f} | ms/batch {:7.2f} |".format(
            ...
            if not save_test_preds:
                test_acc = (
                    evaluate_accuracy(dataset.test(), model)
                    if args.test_statements
                    else 0.0
                )
            else:
            ...
            print("-" * 71)
            print(
                "| epoch {:3} | step {:5} | dev_acc {:7.4f} | test_acc {:7.4f} |".format(
            ...
            start_time = time.time()
            if (
                epoch_id > args.unfreeze_epoch
                and epoch_id - best_dev_epoch >= args.max_epochs_before_stop
            ):
                break
...
def eval_detail(args):
    ...
    if not save_test_preds:
        test_acc = (
            evaluate_accuracy(dataset.test(), model) if args.test_statements else 0.0
        )
    else:
        eval_set = dataset.test()
        total_acc = []
        ...
        with open(preds_path, "w") as f_preds:
            with torch.no_grad():
                for qids, labels, *input_data in tqdm(eval_set):
                    count += 1
                    logits, _, concept_ids, node_type_ids, edge_index, edge_type = (
                        model(*input_data, detail=True)
                    )
                    predictions = logits.argmax(1)  # [bsize, ]
                    preds_ranked = (-logits).argsort(1)  # [bsize, n_choices]
